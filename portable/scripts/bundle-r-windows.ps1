...
# Arguments property accepts one Windows command-line string, so quote each
# argument according to the CommandLineToArgvW/MSVC rules.
...
function Invoke-RValidationProbe {
    ...
    try {
        try {
            # Explicit R_HOME and a working directory inside the staged tree
            # prevent registry state or an unrelated system R installation from
            # influencing startup before the staged runtime is promoted.
...
function Test-RRuntimeProcesses {
    ...
    try {
        ...
        $expressionProbe = Invoke-RValidationProbe -FilePath $rscriptPath -ArgumentList @("--vanilla", $probeScriptPath) -Label "Rscript.exe --vanilla <version-probe.R>" -LogDirectory $LogDirectory -EventName "$EventPrefix probe R version script" -RHome $RHome
...
if (-not (Test-Path $ProjectGoCache)) {
    Write-Host "No source GO cache found - portable prebuild will fill required cache if possible."
}

if ((Test-Path $ProjectBioMartCache) -and
    ((-not (Test-Path $PortableBioMart)) -or
     ($null -eq (Get-ChildItem -LiteralPath $PortableBioMart -Force -ErrorAction SilentlyContinue | Select-Object -First 1)))) {
    ...
    Write-Host "Existing portable BioMart cache retained, or no source cache is available."
}

Write-Host "--- Validating and filling portable AnnotationHub/GO caches ---"
Invoke-WithCleanREnvironment -Environment @{ R_LIBS_USER = $RLibrary } -Action {
    & $RscriptPath --vanilla (Join-Path $ScriptDir "prebuild-cache.R") $GoCache $RLibrary $ProjectGoCache
}
if ($LASTEXITCODE -ne 0) {
