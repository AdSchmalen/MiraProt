...
render_GSEA_tech_core_dataproc_content_GSEA <- function() {
  div(
    ...
    tags$ul(
      ...
      tags$li(code("input$RankinkMethod_GSEA"), ": single dispatch control; ", code("log2 Ratio (precalculated)"), ", ", code("log2 Ratio x -log10(p-Value)"), ", and ", code("-log10(p-Value)"), " select ", code("compute_precalculated_ranks_GSEA()"), " and are passed directly as its ", code("metric"), ". All other values resolve through ", code("rank_methods"), " and select ", code("compute_custom_ranks_GSEA()"), "."),
