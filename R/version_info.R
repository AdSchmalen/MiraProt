...
#
# VERSION is release metadata. Git and BUILD_INFO provide separate source
# revision metadata, and never alter the release version.

.miraprot_read_version <- function(path = "VERSION") {
  if (!file.exists(path) || dir.exists(path)) {
    stop("MiraProt release VERSION file is missing: ", path, call. = FALSE)
  }
  lines <- readLines(path, warn = FALSE)
  if (length(lines) != 1L || !grepl("^[0-9]+\\.[0-9]+\\.[0-9]+$", lines[[1L]])) {
    stop("Invalid MiraProt VERSION (expected exactly MAJOR.MINOR.PATCH): ", path,
         call. = FALSE)
  }
  lines[[1L]]
}
